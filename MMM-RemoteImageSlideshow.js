...
Module.register("MMM-RemoteImageSlideshow", {
  ...
  start: function () {
    ...
    if (this.config.imagePaths.length == 0) {
      this.errorMessage = "MMM-RemoteImageSlideshow: Missing required parameter."
    }
    else {
      ...
      this.sendSocketNotification('REMOTEIMAGESLIDESHOW_REGISTER_CONFIG', this.config);
  ...
  getStyles: function() {
    ...
    return ["remoteimageslideshow.css"];
  },    
  // the socket handler
  socketNotificationReceived: function(notification, payload) {
    // if an update was received
    if (notification === "REMOTEIMAGESLIDESHOW_FILELIST") {
